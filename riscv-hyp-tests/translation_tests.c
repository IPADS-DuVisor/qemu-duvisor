...
DECLARE_TEST(two_stage_translation){
    TEST_START();
    reset_state();
    ...
    goto_priv(PRIV_HS);
    sfence();
    CSRS(sedeleg, (1ULL << 0x18) | (1ULL << 10));
    goto_priv(PRIV_HU);
    hupt_init();
    ...
    goto_priv(PRIV_HU);
    hupt_switch();
    printf("before hufence\n");
    hufence();
    printf("after hufence\n");
    goto_priv(PRIV_VS);
    ...
    goto_priv(PRIV_HS);
    CSRS(CSR_SEDELEG, 1ull << CAUSE_LGPF);
    goto_priv(PRIV_HU);
    goto_priv(PRIV_VS);
    TEST_SETUP_EXCEPT();
    read64(vs_page_base(VSRWX_GI));    
    DEBUG("load guest page fault on unmapped address"
        " excpt.triggered %d "
       " excpt.cause %ld "
        "excpt.tval2 0x%lx "
        "excpt.priv %ld "
    ...
    // CSRS(CSR_SEDELEG, 1 << CAUSE_LPF | 1 << CAUSE_LGPF);
    goto_priv(PRIV_HU);
    goto_priv(PRIV_VS);
    ...
    DEBUG("vs_page_base(VSI_GI) 0x%lx, excpt.tval 0x%lx\n", vs_page_base(VSI_GI), excpt.tval);  
    TEST_ASSERT(
        "invalid pte in both stages leads to s1 page fault", 
        excpt.triggered == true && 
        excpt.cause == CAUSE_LPF &&
        excpt.tval == vs_page_base(VSI_GI) &&
        excpt.priv == PRIV_VS && 
        excpt.gva == false
    );
...
DECLARE_TEST(second_stage_only_translation){
     ...
     */
    goto_priv(PRIV_HS);
    CSRS(CSR_SEDELEG, (1 << CAUSE_LGPF) | (1 << CAUSE_SGPF));
    goto_priv(PRIV_HU);
    hupt_init();
    ...
    DEBUG("access top 0x%lx", vs_page_base_limit(TOP));
    TEST_ASSERT(
    ...
    DEBUG("access top 0x%lx", vs_page_base_limit(TOP) | (1ULL << 41));
    TEST_ASSERT(
        "access top of guest pa space with high bits =/= 0", 
        excpt.triggered == true &&
        excpt.cause == CAUSE_LGPF
...
DECLARE_TEST(no_stage_translation){
    /**
     * Test only no stage translation.
     */
    TEST_START();
...
DECLARE_TEST(m_and_hs_using_vs_access){
...
DECLARE_TEST(hu_using_vs_access){
    ...
    //TODO: test mprv to access hu space

    goto_priv(PRIV_HS);
    goto_priv(PRIV_HU);
    set_prev_priv(PRIV_VS);
    TEST_SETUP_EXCEPT();
    husvd(vaddr, 0xdeadbeef);
    val = hulvd(vaddr);
    TEST_ASSERT("hu hulvd",
        excpt.triggered == false && val == 0xdeadbeef
    );

    TEST_SETUP_EXCEPT();
    husvb(vaddr, ((uint8_t)-1));
    val = hulvb(vaddr);
    valu = hulvbu(vaddr);
    TEST_ASSERT("hu hulvb vs hulvbu",
        excpt.triggered == false && val == (-1) && valu == ((uint8_t)-1)
    );

    TEST_SETUP_EXCEPT();
    husvh(vaddr, ((uint16_t)-1));
    val = hulvh(vaddr);
    valu = hulvhu(vaddr);
    TEST_ASSERT("hu hulvh vs hulvhu",
        excpt.triggered == false && val == (-1) && valu == ((uint16_t)-1)
    );

    TEST_SETUP_EXCEPT();
    husvw(vaddr, ((uint32_t)-1));
    val = hulvw(vaddr);
    valu = hulvwu(vaddr);
    TEST_ASSERT("hu hulvw vs hulvwu",
        excpt.triggered == false && val == (-1) && valu == ((uint32_t)-1)
    );

    /**
     * Some tests are commented out because qemu behaves in a weird way if you
     * try to use a hulvx instruction on a page without read permissions. It
     * gives you a load access fault, with the mepc not even pointing to the
     * hulvx instruction but on a previous lui instruction. 
     * TODO: find out why
     */

    // TEST_SETUP_EXCEPT();
    // val = hulvxwu(vs_page_base(VSX_GUX));
    // TEST_ASSERT("hu hulvxwu accesses on only execute page",
    //     excpt.triggered == false
    // );

    // TEST_SETUP_EXCEPT();
    // val = hulvxwu(vs_page_base(VSRWX_GURWX));
    // TEST_ASSERT("hu hulvxwu accesses page with all permissions",
    //     excpt.triggered == false
    // );

    // TEST_SETUP_EXCEPT();
    // val = hulvxwu(vs_page_base(VSRWX_GURW));
    // TEST_ASSERT("hu hulvxwu on hu-level non-exec page leads to lgpf",
    //     excpt.triggered == true && 
    //     excpt.cause == CAUSE_LGPF && 
    //     excpt.gva == true &&
    //     excpt.xpv == false
    ...
    // TEST_SETUP_EXCEPT();
    // val = hulvxwu(vs_page_base(VSRW_GURWX));
    // TEST_ASSERT("hus hulvxwu on vs-level non-exec page leads to lpf",
    //     excpt.triggered == true && 
    //     excpt.cause == CAUSE_LPF  && 
    //     excpt.gva == true &&
    //     excpt.xpv == false
    ...
    write64(addr, 0x1107ec0ffee);
    val = hulvd(vaddr);
    TEST_ASSERT("hu hulvd to vu page leads to exception when spvp = 1",
        excpt.triggered == true
    );
    ...
    write64(addr, 0x1107ec0ffee);
    val = hulvd(vaddr);
    TEST_ASSERT("hu hulvd to vu page successful when spvp = 0",
        excpt.triggered == false
    );

    CSRS(CSR_HUVSSTATUS, SSTATUS_SUM);
    goto_priv(PRIV_M);
    ...
    write64(addr, 0x1107ec0ffee);
    val = hulvd(vaddr);
    TEST_ASSERT("hu hulvd to user page successful when vsstatus.sum set",
        excpt.triggered == false && val == 0x1107ec0ffee
    );
    CSRC(CSR_VSSTATUS, SSTATUS_SUM);
    ...
    TEST_SETUP_EXCEPT();
    val = hulvd(vaddr);
    TEST_ASSERT("hu hulvd of xo vs page leads to exception",
        excpt.triggered == true
    );
    ...
    TEST_SETUP_EXCEPT();
    val = hulvd(vaddr);
    TEST_ASSERT("hu hulvd of xo vs page succsseful",
        excpt.triggered == false
    );
    goto_priv(PRIV_HS);
    CSRC(sstatus, SSTATUS_MXR);
    ...
    TEST_SETUP_EXCEPT();
    val = hulvd(vaddr);
    TEST_ASSERT("hu hulvd of xo vs page leads to load page fault",
        excpt.triggered == true &&
        excpt.cause == CAUSE_LPF
    );
    ...
    TEST_SETUP_EXCEPT();
    CSRS(CSR_HUVSSTATUS, SSTATUS_MXR);
    val = hulvd(vaddr);
    TEST_ASSERT("hu hulvd of xo vs page succsseful with sstatus.mxr set",
        excpt.triggered == false
    );
    ...
    vaddr = vs_page_base(VSI_GUR) + 1;
    TEST_SETUP_EXCEPT();
    husvb(vaddr, 0xdeadbeef);
    TEST_ASSERT("hu husvb on ro 2-stage page leads to store guest page fault",
        excpt.triggered == true &&  
        excpt.cause == CAUSE_SGPF &&
        excpt.tval2 == vaddr >> 2
    );
    ...
    TEST_SETUP_EXCEPT();
    val = hulvb(vaddr);
    TEST_ASSERT("hu hulvb on ro 2-stage page successfull",
        excpt.triggered == false
    );
    ...
    // CSRW(sscratch, 0x911);
    husvb(vaddr, 0xdeadbeef);
    TEST_ASSERT("hu husvb on ro both stage page leads to store page fault",
        excpt.triggered == true &&  
        excpt.cause == CAUSE_SPF
    );
    ...
    // CSRW(sscratch, 0x112);
    husvb(vaddr, 0xdeadbeef);
    TEST_ASSERT("hu husvb on invalid 2 stage page leads to store guest page fault",
        excpt.triggered == true &&  
        excpt.cause == CAUSE_SGPF
    );
