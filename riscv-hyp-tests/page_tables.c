...
pte_t hupt_root[PAGE_SIZE*4/sizeof(pte_t)] __attribute__((aligned(PAGE_SIZE*4)));
pte_t hupt[5][PAGE_SIZE/sizeof(pte_t)] __attribute__((aligned(PAGE_SIZE)));
...
void hupt_init(){
    ...
    hupt_root[MEM_BASE/SUPERPAGE_SIZE(0)] =
        PTE_V | (((uintptr_t)&hupt[0][0]) >> 2);

    addr = MEM_BASE;
    for(int i = 0; i < 512; i++) hupt[0][i] = 0;
    for(int i = 0; i < MEM_SIZE/SUPERPAGE_SIZE(1)/2; i++){
        hupt[0][i] = 
            PTE_V | PTE_U | PTE_AD | PTE_RWX | (addr >> 2);  
        addr +=  SUPERPAGE_SIZE(1);
    ...
    hupt_root[4] =
        PTE_V | (((uintptr_t)&hupt[1][0]) >> 2);

    hupt_root[2047] =
        PTE_V | (((uintptr_t)&hupt[1][0]) >> 2);

    hupt[1][0] = 
        PTE_V | (((uintptr_t)&hupt[2][0]) >> 2);
    ...
    addr = TEST_PPAGE_BASE;
    for(int i = 0; i < TEST_PAGE_MAX; i++){
        hupt[2][i] = (addr >> 2) | PTE_AD |
            test_page_perm_table[i].h;  
        addr +=  PAGE_SIZE;
    }

    hupt[1][1] = 
        PTE_V | (((uintptr_t)&hupt[3][0]) >> 2);
    addr = TEST_PPAGE_BASE;
    for(int i = 0; i < 512; i++){
        hupt[3][i] = (addr >> 2) | 
            PTE_V | PTE_U | PTE_AD | PTE_RWX; 
    ...
    // if(curr_priv == PRIV_HS || curr_priv == PRIV_M){
        uintptr_t husatp = (((uintptr_t)hupt_root) >> 12) | (0x8ULL << 60);
        CSRW(CSR_HUGATP, husatp);
    // } else {
    //     ERROR("trying to set hs hugatp from lower privilege");
    // }
}
...
void hupt_switch(){
    pte_t temp = hupt[2][SWITCH1];
    hupt[2][SWITCH1] = hupt[2][SWITCH2];
    hupt[2][SWITCH2] = temp;
}
