...
enum ecall {ECALL_GOTO_PRIV = 1, ECALL_TESTING};
...
typedef bool (*test_func_t)(uint64_t *out_p, uint64_t *out_f);
extern test_func_t* test_table;
extern size_t test_table_size;

#define DECLARE_TEST(name)\
    bool name(uint64_t *out_p, uint64_t *out_f)

#define TEST_START()\
    const char* __test_name = __func__;\
    bool test_status = true;\
    uint64_t nr_passed = 0, nr_failed = 0;\
    printf(CCYN "%s ----->\n" CDFLT, __test_name);

#define TEST_REGISTER(test)\
    bool test(uint64_t *, uint64_t *);\
...
#define TEST_ASSERT(test, cond, ...) {\
    if(LOG_LEVEL >= LOG_DETAIL){\
        ...
        printf(CMAG "\t%-70.*s" CDFLT, line_size, test);\
        for(int i = line_size; i < size; i+=line_size)\
            printf(CMAG "\n\t%-70.*s" CDFLT, line_size, &test[i]);\
        if(cond) \
            printf(CGRN "PASSED" CDFLT);\
        else\
            printf(CRED "FAILED" CDFLT);\
        if(!(cond)) { printf("\n\t("); printf(""__VA_ARGS__); printf(")"); }\
        printf("\n");\
    }\
    if (cond) nr_passed++; else nr_failed++;\
...
failed:\
    ...
    printf("\n");\
    if (nr_failed == 0)\
        printf(CGRN "<----- %s TEST OK! [%lu / %lu] PASSED\n" CDFLT, \
                __test_name, nr_passed, nr_passed + nr_failed);\
    else\
        printf(CRED "<----- %s TEST FAILED! [%lu / %lu] FAILED\n" CDFLT, \
                __test_name, nr_failed, nr_passed + nr_failed);\
    printf("\n");\
    *out_p = nr_passed;\
    *out_f = nr_failed;\
    return (test_status);\
